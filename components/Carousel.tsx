...
const slides = [
  { id: 1, image: "/images/tobacco-1.webp", title: "Hand-Rolled Craft", subtitle: "Tradition refined with modern design" },
  { id: 2, image: "/images/tobacco-2.jpg",  title: "Aged to Perfection",  subtitle: "Warm tones, precise finishes, timeless feel" },
  { id: 3, image: "/images/tobacco-3.jpg",  title: "Artisan Materials",    subtitle: "Stone, wood & brass — built to last" },
];
  ...
  return (
    <section className="relative w-full h-[750px] pt-32 py-16 overflow-hidden bg-[#f7f3e9]">
      <div className="absolute top-10 left-1/2 transform -translate-x-1/2 text-center z-20">
        <h1 className="text-4xl font-bold text-amber-700 mb-8">Our Heritage Collection</h1>
      </div>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isActive ? "scale-100 z-20 opacity-100" : "scale-75 opacity-60"}
            ...
            >
              <div className="relative w-[90vw] sm:w-[700px] h-[450px] sm:h-[500px] rounded-2xl overflow-hidden shadow-2xl">
                ...
                <div className="absolute bottom-0 w-full p-6 backdrop-blur-sm bg-stone-900/30 rounded-t-2xl">
                  <h2 className="text-3xl font-bold text-amber-100 drop-shadow">{s.title}</h2>
      ...
      <div className="absolute top-1/2 left-6 transform -translate-y-1/2 z-20 cursor-pointer" onClick={handlePrev}>
      ...
      <div className="absolute top-1/2 right-6 transform -translate-y-1/2 z-20 cursor-pointer" onClick={handleNext}>
        <button className="w-12 h-12 flex items-center justify-center rounded-full bg-[#fdfaf5] shadow-md border border-stone-200 hover:bg-amber-100 transition">
          <span className="text-2xl text-amber-700">›</span>
        </button>
      </div>
      ...
      <div className="absolute bottom-6 flex justify-center gap-2 w-full">
        {slides.map((_, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
            className={`h-3 w-3 rounded-full transition-all ${active === i ? "bg-amber-500" : "bg-stone-600"}`}
          />
        ))}
      </div>
    </section>
